...
│   ├── WORKSPACE_DESIGN.md  # 完整目录规范（AGENTS.md 里是摘要）
│   ├── BOOTSTRAP.md.tpl     # 每实例差异模板，由 .env 生成 BOOTSTRAP.md
...
> `SOUL.md`、目录规范 `WORKSPACE_DESIGN.md`、每实例引导 `BOOTSTRAP.md.tpl` 等），
> 由 `init.sh` 一次性种入新实例，取代了过去"手动把规则粘贴给 agent"的做法。详见
