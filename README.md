...
├── ollama-embedding/        # memorySearch embedding 服务（所有实例共用，按需启动）
│   ├── docker-compose.yml
│   └── README.md
├── scheduler/               # 定时任务容器镜像（supercronic，不消耗 LLM token）
│   ├── Dockerfile
│   ├── entrypoint.sh
│   ├── run-job.sh
│   ├── USAGE.md
│   └── README.md
├── custom-skills/           # 自定义 skill 源码（init.sh/update.sh 自动同步到各实例）
│   ├── README.md
│   └── <skill-name>/
├── .env.example             # 环境变量模板（复制为 .env 后填写真实值）
├── init.sh                  # 初始化脚本（首次部署用，含防重复 lock）
├── update.sh                # 模板有更新时，同步已初始化实例的 docker-compose.yml 及 custom-skills/
...
详细配置和验证方法见 `ollama-embedding/README.md`。
...
如果一个自定义 skill 要在多个实例间共享，把源码放进仓库的 `custom-skills/<skill-name>/`
...
改 skill 只需要改 `custom-skills/` 里的源文件，再对各实例跑一遍 `update.sh`。详见
