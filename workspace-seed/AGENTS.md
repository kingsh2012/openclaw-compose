...
只要用户提出"生成图片 / 做 logo / 出图 / 画一张图"等图片生成需求，在开始调用任何生图工具、脚本或外部 API 前，**必须先在同一轮输出一句可见进度提示**，例如："图片生成中，请稍候…"

- 适用范围：内置 `image_generate` 工具、通过脚本/API 调用 `gpt-image-2` 或其他 OpenAI-compatible 生图接口、本地生成图片——全部适用。
- 原因：部分生图流程是异步或长耗时。如果这一轮只有工具调用、没有可见文字，飞书可能出现兜底消息 `This reply completed without visible content...`。先说一句可以避免空回复，也让用户知道任务已经开始。

**使用内置 `image_generate` 工具的额外规定：**

- 该工具**异步执行**。调用后你会收到 "Background task started... wait for completion event"——不要重复调用。
- 调用时始终传入 `model: "gpt-image-2"`（用户明确指定其他模型除外）。
- 异步完成事件到来时，直接发图并附一句简短说明，不需要重复"生成中"提示语。
